 ...
 *     ├── question_expander       — breaks topic into sub-questions, edge cases, angles
 *     ├── dynamic_research_squad  — spawns one researcher per sub-question, all in parallel
 *     └── report_composer         — compiles all research into final report
 */

import { LlmAgent, SequentialAgent } from "@google/adk";
import { DynamicResearchSquad } from "./dynamic-research-squad";
...
export function createDeepResearchAgent(config: AgentConfig) {
  // ─── Query Analyzer (first step — enriches the raw user query) ────────────

  const queryAnalyzer = new LlmAgent({
    name: "query_analyzer",
    model: config.fastModel,
    description:
      "Analyzes and enriches the user's raw query with organizational context for more effective research.",
...
Do NOT answer the question — only analyze and enrich it for downstream agents.`,
    outputKey: "enriched_query",
  });

  // ─── Question Expander ─────────────────────────────────────────────────────

  const questionExpander = new LlmAgent({
    name: "question_expander",
    model: config.advancedModel,
    description:
      "Expands the enriched query into exactly 5 detailed sub-questions for parallel research.",
...
5. Who is responsible for enforcing V?`,
    outputKey: "research_questions",
  });

  // ─── Dynamic Research Squad ────────────────────────────────────────────────

  const researchSquad = new DynamicResearchSquad({
  ...
  const researchCompiler = new LlmAgent({
    name: "research_compiler",
    model: config.reportModel,
    description:
      "Compiles all research findings and sources into a single organized output.",
...
Do not add a memo header. Do not reference the research process.`,
  ...
  // ─── Full Pipeline (SequentialAgent) ────────────────────────────────────
  // Auto-executes: query analysis → question expansion → dynamic parallel research → report

  const rootAgent = new SequentialAgent({
    name: "deep_research_pipeline",
    description:
      "Full auto-executing deep research pipeline: query analysis, question expansion, dynamic per-question parallel research, and final report composition.",
    subAgents: [
      queryAnalyzer,
      questionExpander,
      researchSquad,
